...
update_cap <-"
UPDATE generated_author_info_clean
SET area = UPPER(area);
"

dbExecute(connection, update_cap)
generated_author_info_clean <- dbGetQuery(connection, 'SELECT * FROM generated_author_info_clean;')

query <-"
SELECT gai.researcher_name AS 'Author Name', gai.dept AS 'Institution', SUM(gai.count) AS 'A* Publication Count'
FROM generated_author_info_clean AS gai
JOIN conference_ranking AS cr
ON gai.area = cr.acronym
JOIN field_conference AS fc
ON cr.acronym = fc.conference_name
WHERE cr.rank = 'A*' AND gai.publication_year BETWEEN 2010 AND 2024
GROUP BY gai.researcher_name, gai.dept
ORDER BY SUM(gai.count) DESC, 
         gai.researcher_name ASC, 
         gai.dept ASC
LIMIT 100;
"
...
query <-"
WITH InstitutionCounts AS (
  SELECT fc.specialized_field AS cs_field, gai.dept AS institution, SUM(gai.count) AS asterisk_count
  FROM generated_author_info_clean AS gai
  JOIN conference_ranking AS cr 
  ON gai.area = cr.acronym
  JOIN field_conference AS fc 
  ON gai.area = fc.conference_name
  WHERE cr.rank = 'A*'
  GROUP BY fc.specialized_field, gai.dept
),
RankedInstitutions AS (
  SELECT cs_field, institution, asterisk_count, RANK() OVER (PARTITION BY cs_field ORDER BY asterisk_count DESC) AS rnk
  FROM InstitutionCounts
)
SELECT 
  cs_field AS 'CS Field', institution AS'Institution Name', asterisk_count AS 'A* Publication Count'
FROM RankedInstitutions
WHERE rnk <= 10
ORDER BY cs_field ASC, asterisk_count DESC, institution ASC;
"
