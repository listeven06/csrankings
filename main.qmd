...
# Assume csranking_clean already exists from earlier steps (with scholarid cleaned)
# (i.e. csranking_clean was produced as:)
# csranking_clean <- csranking %>%
#   mutate(scholarid = ifelse(scholarid == "NOSCHOLARPAGE", NA, scholarid))
...
csranking_keyed <- csranking_clean %>%
...
csranking_clean <- csranking_keyed %>%
  group_by(merge_key) %>%
  summarise(
...
if (row_count$count == 0) {
  # Only load the data if the table is empty
  dbWriteTable(connection, "csrankings_clean", csranking_clean, append = TRUE, row.names = FALSE)
